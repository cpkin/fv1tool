 ...
 *
 * Key improvements over the previous SpinGPT interpreter:
...
// ---------------------------------------------------------------------------
// Register mapping: SpinGPT compiled register indices → internal 64-slot layout
//
...
// ADDR_PTR (SpinGPT index 24) → stays at internal 24
REG_MAP[24] = 24;
// Special registers
REG_MAP[32] = 20; // ADCL
REG_MAP[33] = 21; // ADCR
...
// Inverse register map: audiofab internal index → SpinGPT compiler index.
...
// Internal 32-63 → SpinGPT 0-31 (REG0-REG31)
...
// LFO parameter registers: wire indices 0-7 map to SpinGPT 43-50
...
/**
 * Convert a SpinGPT compiled delay address to a delayPtr-relative offset.
 *
 * SpinGPT stores delay addresses in two forms:
...
export class FV1Core {
  ...
  private runPass(inL: number, inR: number, lr: number): void {
    // PACC = previous sample's final ACC (SpinGPT convention)
  ...
  private exec(inst: CompiledInstruction, pc: number): number | null {
    ...
    switch (inst.opcode) {
      ...
      case 'skp_neg':
      case 'skp_zrc': {
        // SpinGPT flags: RUN=0x01, ZRO=0x02, GEZ=0x04, NEG=0x08, ZRC=0x10
        const flags = o[0];
        const n = o[1];
  ...
  private skpCondMet(flags: number): boolean {
    // SpinGPT flag encoding: RUN=0x01, ZRO=0x02, GEZ=0x04, NEG=0x08, ZRC=0x10
...
function decodeRawWord(word: number): CompiledInstruction | null {
  ...
  // Wire-format register → SpinGPT compiler index (so REG_MAP in exec() maps back correctly)
  ...
  switch (op) {
    ...
    case 0x11: {
      // SKP / JMP — decode raw hardware flag bits → SpinGPT internal flag encoding
      const rawFlags = (word >>> 27) & 0x1f;
      // Hardware bits: [31]=RUN [30]=ZRC [29]=ZRO [28]=GEZ [27]=NEG
      // SpinGPT bits:  RUN=0x01  ZRO=0x02  GEZ=0x04  NEG=0x08  ZRC=0x10
      let flags = 0;
    ...
    case 0x14: {
      ...
      // CHO mode mapping: wire 0=RDA, 2=SOF, 3=RDAL → SpinGPT 0,1,2
